...
function getRepeatCount(repeatMapList, count) {
    return repeatMapList
        .reduce((sum, map) => {
            return hasRepeatCount(map, count) ? sum + 1 : sum;
        }, 0)
}

function hasRepeatCount(map, count) {
    for (let key in map) {
        if (map.hasOwnProperty(key) && map[key] === count) {
            return true;
        }
    }
    return false;
...
function getRepeatMapList(boxIds) {
    return boxIds
        .reduce((list, boxId) => {
            list.push(getRepeatMap(boxId));
            return list;
        }, []);
}

function getRepeatMap(boxId) {
    return boxId.split('')
